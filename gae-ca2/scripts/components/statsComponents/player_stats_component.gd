...
# Updates the specific multipliers 
func apply_add_modifier(stat_name:String, value: float):
	additive_mods[stat_name] = additive_mods.get(stat_name, 0) + value

# Updates the specific multipliers 
func apply_mult_modifier(stat_name:String, value):
	multiplicative_mods[stat_name] = multiplicative_mods.get(stat_name, 0) + value
